...
option_list <- list(
  ...
  make_option(c("-r", "--rdata"), type="character", default="data.Rdata", help="Output directory [default: %default]")
)
...
DElist <- function(deseqObject, numerator, denominator, lfcCutoff, pvalueCutoff, condCol) {
  res <- results(deseqObject,
                 ...
                 alpha = pvalueCutoff,  # Using pvalue instead of padj
  ...
  res$Regulation <- "Not Significant"  # default label
  res$Regulation[res$log2FoldChange > 1 & res$padj < 0.05] <- "Upregulated"
  res$Regulation[res$log2FoldChange < -1 & res$padj < 0.05] <- "Downregulated"
  ...
  volcanoPlot <- EnhancedVolcano(res, lab = res$IDS,
                                  x = 'log2FoldChange', y = 'pvalue',  # Change y to pvalue
                                  title = paste0(numerator, " VS ", denominator),
                                  subtitle = NULL,
                                  caption = "",  # Hides the EnhancedVolcano caption
                                  pCutoff = pvalueCutoff, FCcutoff = lfcCutoff,
  ...
  # Apply cutoff based on p-value instead of adjusted p-value
...
heat_mat <- t(apply(heat_mat, 1, as.numeric))  # preserves rows as genes
...
# Save entire R session environment
